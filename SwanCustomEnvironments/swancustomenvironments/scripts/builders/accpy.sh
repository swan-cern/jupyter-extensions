...
uv pip install -i https://acc-py-repo.cern.ch/repository/vr-py-releases/simple --allow-insecure-host acc-py-repo.cern.ch -r "${REQ_PATH}" "ipykernel==${IPYKERNEL_VERSION}" ${SPARKCONNECTOR} ${SPARKMONITOR} ${NXCALS} | tee -a ${LOG_FILE}
