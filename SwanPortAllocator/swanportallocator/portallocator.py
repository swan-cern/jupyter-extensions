...
import contextlib
from socket import socket, SO_REUSEADDR, SOL_SOCKET, gethostname, error as SocketError
...
class PortAllocator(threading.Thread):
    ...
    def check_given_ports_status(self):
        ...
        for process in list(self.clients.keys()):
            ...
            if (
                self.clients[process]["status"] == Conn_State.CONNECTING.value
                and self.clients[process]["time"] + 60 < time.time()
            ):
                self._check_process(process)
...
class PortAllocatorClient:
    ...
    def get_ports(self, n):
        ...
        """
        request_info = {"action": Actions.GET_PORT.value, "process": self.pid, "n": n}
    ...
    def release_ports(self, ports):
        ...
        """
        request_info = {"action": Actions.RELEASE_PORT.value, "process": self.pid, "ports": ports}
        self.socket.send_json(request_info)
        self.socket.recv_json()

    def _set_status(self, status):
        request_info = {"action": Actions.SET_STATUS.value, "process": self.pid, "status": status}
        self.socket.send_json(request_info)
        self.socket.recv_json()
