...
def has_good_chars(name, extra_chars=""):
    ...
    allowed = string.ascii_lowercase + string.ascii_uppercase + string.digits + "/._+-" + extra_chars
...
def check_url(url):
    ...
    if not is_good_server:
        raise web.HTTPError(
            400,
            "The URL of the project is not a github, CERN gitlab, CERNBox shared link nor root.cern.ch URL. It is not a path on EOS either.",
        )
