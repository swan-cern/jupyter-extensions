...
class ProjectsMixin(HasTraits):
    swan_default_folder = Unicode("SWAN_projects", config=True, help="")

    swan_default_file = Unicode(".swanproject", config=True, help="")

    untitled_project = Unicode("Project", config=True, help="The base name used when creating untitled projects.")
    ...
    async def new(self, model=None, path=""):
        if model and "type" in model and "ext" in model and model["type"] == "directory" and model["ext"] == "project":
            model["is_project"] = True
            model["ext"] = ""
