...
class SwanShare(Configurable):
    token_file = Unicode("/tmp/swan_oauth.token", config=True, help="Path to the token file used for authenticate")

    cernbox_url = Unicode("https://cernbox.cern.ch", config=True, help="URL of the CERNBox instance to auth")
...
class SwanShareHandler(APIHandler):
    ...
    def get(self):
        ...
        with open(self.config.token_file, "r") as f:
            ...
            r = requests.get(
                f"{self.config.cernbox_url}/swanapi/v2/authenticate",
...
def _load_jupyter_server_extension(serverapp):
    ...
    host_pattern = ".*$"
    route_pattern = url_path_join(web_app.settings["base_url"], r"/api/swanshare")
